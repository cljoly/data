...
#	| ~/.local/bin/sqlite-utils insert tmp.db repo -
#
#curl \
#	-H "Accept: application/vnd.github.mercy-preview+json" \
...
~/.local/bin/sqlite-utils create-view tmp.db topic "SELECT DISTINCT \
	repo.rowid as rid, json_each.value as t \
	FROM repo, json_each(topics)"

~/.local/bin/sqlite-utils create-view tmp.db featured_repo "SELECT DISTINCT \
...
~/.local/bin/sqlite-utils query tmp.db --json-cols "SELECT DISTINCT \
	* \
	FROM featured_repo \
